...
for PORT in ${PORTS}; do
  ...
  EXPOSE_PORT=$(curl --unix-socket /var/run/docker.sock http://localhost/containers/a872585638c5/json  | jq .NetworkSettings.Ports | jq '.["${PORT}/tcp"]'[0].HostPort | jq tonumber)
  EXPOSE_CPORT=$(curl --unix-socket /var/run/docker.sock http://localhost/containers/a872585638c5/json  | jq .NetworkSettings.Ports | jq '.["${CPORT}/tcp"]'[0].HostPort | jq tonumber)
